...
namespace ChillX.Serialization.Benchmark.MessagePackEntity
{
    ...
    public class BenchMessagePack : BenchBase
    {
        ...
        //private ThreadSafeQueue<MessagePackEntity.TestClassVariantA> Queue_ChillX = new ThreadSafeQueue<MessagePackEntity.TestClassVariantA>();

        private ThreadSafeQueue<byte[]> Queue_Buffer = new ThreadSafeQueue<byte[]>();
        ...
        private MessagePackEntity.TestClassVariantA TestClassOne = TestClassOne_Create(rnd, 64);
        private static MessagePackEntity.TestClassVariantA TestClassOne_Create(Random rnd, int stringSize)
        {
            return new MessagePackEntity.TestClassVariantA().RandomizeData(rnd, stringSize);
        }
        ...
        {
            MessagePackEntity.TestClassVariantA TestClassInstance = TestClassOne.Clone();
            int Counter = 0;
            for (int I = 0; I < numReps; I++)
            {
                ...
                //    Thread.Sleep(0);
                //}
                Queue_Buffer.Enqueue(MessagePack.MessagePackSerializer.Serialize<MessagePackEntity.TestClassVariantA>(TestClassInstance));
                Counter++;
                //if (Counter > 100)
                //{
        ...
        protected override void Subscribe()
        {
            ...
            while (true)
            {
                ...
                if (buffer != null)
                {
                    TestClassInstance = MessagePack.MessagePackSerializer.Deserialize<MessagePackEntity.TestClassVariantA>(buffer);
                    //lock (SizeLock) { pendingSize--; }
                }
                else
                {
